...
if sudo docker images -q test_car >/dev/null 2>&1 ; then
    ...
    ~/jetcar/test/start.sh
fi

docker run -it --rm --device=/dev/ttyUSB0:/dev/ttyUSB0 --device=/dev/video0:/dev/video0 -p 8888:8888 -v /home/adam/jetcar/test/notebooks:/root/notebooks test_car
#